...
const REDACTED_USDA_KEY = process.env.REDACTED_USDA_KEY || 'REDACTED_USDA_KEY';
...
export async function searchUSDAFDC(query: string): Promise<any> {
  try {
    ...
    const searchResponse = await fetch(USDA_API_URL + "/foods/search?query=" + encodeURIComponent(query) + "&api_key=" + REDACTED_USDA_KEY);
    ...
    const detailsResponse = await fetch(USDA_API_URL + "/food/" + foodId + "?api_key=" + REDACTED_USDA_KEY);
